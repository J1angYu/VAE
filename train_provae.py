...
from models.ProVAE import ProVAE, vae_bce_loss
...
def train_stage(model, train_loader, optimizer, device, stage, args):
    """训练单个阶段"""
    ...
    for epoch in range(n_epochs):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            if stage > 0 and n_fade > 0 and epoch < n_fade:
                progress = (epoch + i / len(train_loader)) / n_fade
...
def main(args):
    ...
    with Logger(os.path.join(exp_dir, 'training.log')):
        ...
        print("开始训练Progressive VAE...")
        
        # 逐阶段训练
        for stage in range(model.max_stage + 1):
            train_stage(model, train_loader, optimizer, device, stage, args)
